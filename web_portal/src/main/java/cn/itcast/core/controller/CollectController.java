...
public class CollectController {
    ...
    public Result addItemToCollectList(Long itemId){
        ...
        if ("anonymousUser".equals(userName)) {
            ...
            if (collectList!=null){
                //3. 将当前商品加入到收藏列表
                collectList = collectService.addItemToCollectList(collectList, itemId);
            }else{
                collectList = new ArrayList<Item>();
              Item item= collectService.createitem(itemId);
                collectList.add(item);
            }
            //4.将收藏列表存入redis中
            collectService.setCollectListToRedis(collectList, userName);
    ...
    public List<Item> findCollectList() {
        ...
        String userName = SecurityContextHolder.getContext().getAuthentication().getName();
            ...
            List<Item> redisCollectList = collectService.getCollectListFromRedis(userName);
